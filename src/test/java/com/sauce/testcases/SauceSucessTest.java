...
import com.sauce.pageobject.SauceSucesspage;

public class SauceSucessTest extends BaseClass{
...
//calliing the Cartpage class by making the object
		...
		else
		{
			...
			{
				logger.info("Verifyoverview - Passed");
				Assert.assertTrue(true);
			}
			else
			{
				logger.info("Verifyoverview - Failed");
				captureScreenShot(driver,"Verifyoverview");
				Assert.assertTrue(false);
			}
			
			Thread.sleep(2000);
			SauceSucesspage complete=new SauceSucesspage(driver);
			complete.Sucessmsg();
